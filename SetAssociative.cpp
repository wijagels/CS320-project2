...
void SetAssociative::check_cache(instr cmd) {
  bool result = check_addr(cmd.second, cmd.first, true);
  if (result)
    hits_++;
  else
    misses_++;
...
bool SetAssociative::check_addr(uint32_t addr, bool type, bool first) {
  if (!first) addr += 1 << 5;
  uint32_t tag = compute_tag(addr);
  uint16_t set = compute_set(addr);
  bool result;
  if (type && no_write_)
  ...
  if (first) {
    if (result) {
      if (prefetch_ && !on_miss_) check_addr(addr, false, false);
    } else {
      if (prefetch_) check_addr(addr, false, false);
    }
  }
  return result;
}
